...
export default function TradeRoomPage() {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-background">
      ...
      {/* Desktop Layout */}
      <div className="hidden lg:flex flex-1 flex-row min-h-0 pt-16">
        {/* Main Chat Area (flex column) */}
        <div className="flex-1 flex flex-col min-h-0">
          {/* Room Info Bar - Sticky below global header */}
          <div className="sticky top-16 z-40 bg-card border-b border-border px-6 py-3 flex-shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <Button variant="ghost" size="icon" onClick={() => router.back()}>
                  <ArrowLeft className="h-5 w-5" />
              ...
              <div className="flex items-center gap-3">
                ...
                <Button 
                  ...
                  size="sm" 
                  onClick={() => setShowSidebar(!showSidebar)}
                  className="font-display text-xs px-3 py-1.5 ml-2"
                >
                  {showSidebar ? <X className="h-4 w-4" /> : <Menu className="h-4 w-4" />}
                </Button>
              </div>
              ...
              {pinnedMessages.length > 0 && (
                <div className="mb-8">
                  ...
                  <div className="space-y-4">
                    {pinnedMessages.map((message) => (
                      <div key={message.id} className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
        ...
        <div className="w-80 bg-card border-l border-border flex flex-col flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-6 min-h-0">
            ...
            <div ref={activeMembersListRef} className="space-y-3 max-h-96 overflow-y-auto">
              {messages.slice(0, 10).map((message) => (
                <div
                  key={message.id}
                  className="flex items-center gap-3 cursor-pointer hover:bg-muted p-2 rounded-lg transition-colors"
